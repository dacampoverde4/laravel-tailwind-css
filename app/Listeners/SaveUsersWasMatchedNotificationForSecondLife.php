    ...
    {       
        ...
        if ($event->matched_user_1 && $event->matched_user_2) {
            ...
            if($event->matched_user_2->displayname && $event->matched_user_1->uuid) {
                ...
                $userLink='['.url('/userprofile/'.$encodeid)." ".$event->matched_user_2->displayname."]";

                $slMsg = "Congratulations ".$event->matched_user_1->displayname."! ".$event->matched_user_2->he_she." Likes you too... ".$userLink." is a match! Join ".$event->matched_user_2->him_her." in a ";
                $slMsg .= '['.url('/chat?id='.$event->matched_user_2->id).' chat]'; 
                $slMsg .= " or schedule a ";
                $slMsg .= '['.url( 'schedule/'.base64_encode($event->matched_user_2->id) ).' Trial Date]';
                $slMsg .= '!';
            ...
            if($event->matched_user_1->displayname && $event->matched_user_2->uuid) {
                ...
                $message = "Congratulations ".$event->matched_user_2->displayname."! Now that you and ".$userotherprofilelink." are a match, Join ".$event->matched_user_1->him_her." in a ";
                ...
                $userLink='['.url('/userprofile/'.$encodeid)." ".$event->matched_user_1->displayname."]";

                $slMsg = "Congratulations ".$event->matched_user_2->displayname."! Now that you and ".$userLink." are a match, Join ".$event->matched_user_1->him_her." in a ";
                $slMsg .= '['.url('/chat?id='.$event->matched_user_1->id).' chat]'; 
                $slMsg .= " or schedule a ";
                $slMsg .= '['.url( 'schedule/'.base64_encode($event->matched_user_1->id) ).' Trial Date]';
                $slMsg .= '!';

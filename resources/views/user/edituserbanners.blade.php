                    ...
                    <form method="POST" action="{{ route('updateuserbanners.manageads', $ads_subs_id) }}" enctype="multipart/form-data">
                        ...
                        @foreach($banners_list as $key=>$banner)
                            ...
                            <div class="row mb10">
                                ...
                                <div class="col-md-6">
                                    <input type="file" id="{{ $banner['id'] }}" name="banner_image[]" onchange="readURL(this);" class="form-control bnnrimg" accept="image/jpeg, image/png" value="{{ $userbanners[$key]['image'] }}" mybanner="{{ $banner['id'] }}" mybannerwidth="{{ $banner['banner_width'] }}" mybannerheight="{{ $banner['banner_height'] }}">

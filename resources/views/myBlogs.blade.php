                    ...
                    <div class="col-md-3"> 
                        <a href="{{route('blogview', $_blog->id )}}"> 
                          @foreach(json_decode($_blog->image) as $img)
                                    @endforeach
                        <a href="{{route('blogview', $_blog->id )}}"><img  src="{{ asset('/uploads/'.$img)}}" title="blog" class="blog"></a> 
     ...
     @if(!empty($blog))
      @foreach($blog as $_blog)
          <div class="mid_bit reapeter">
           ...
           @foreach($Category as $cate)
           @php 
             $catdata=\App\Blog::where('category_id',$cate->id)->where('bloger_id',\Auth::user()->id)->where('is_approve',1)->get();
   ...
   <div class="col-md-4 blog-sider">
            ...
            <div class="container">
              <div class="row" style="margin: 20px">
                <p>Added new blogs from here...</p>
                <a href="{{url('/bloggerAdd')}}">New Blog</a>
              </div>
            </div>
            @endif
